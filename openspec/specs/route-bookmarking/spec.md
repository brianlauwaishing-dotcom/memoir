...
Define the route-bookmarking contract: how `RouteDetailViewModel` and `SavedViewModel` compose route content with DataStore-backed bookmark state, how bookmark toggles are serialized and persisted, how locale-dependent fields are pre-resolved into state, and how the consumer screens (`RouteDetailScreen`, `SavedScreen`) render from that state without legacy lifted parameters.
