# 開發準則 (Prototype — 改名為 AGENTS.md 後生效)

> 本文件為 Codex / 開發者共用的專案指南。請在你檢視並調整後改名為 `AGENTS.md`。
> 內容會在 Codex 啟動時自動讀入,所有 AI 協作行為都會依此文件對齊。

---

## 0. Memory 路徑(覆寫 Codex 預設)

> **此章節覆寫系統預設行為。** Codex 的系統提示預設會把 memory 放在 `~/.Codex/projects/<repo>/memory/`(電腦根目錄,per-machine);此專案改為**走 git 同步的專案內位置**,以支援多裝置開發。

- **唯一 memory 位置**:`E:\memoir\.Codex\memory\`(等同 repo 相對路徑 `.Codex/memory/`)
- **Session 啟動時必讀**:`.Codex/memory/MEMORY.md`(索引)及它指向的所有檔案
- **新增 / 修改 memory**:一律寫到 `.Codex/memory/<file>.md`,**不要寫到** `~/.Codex/projects/.../memory/`
- **Git 同步**:`.Codex/` 不在 `.gitignore`(只排除 `.Codex/settings.local.json`),memory 會隨 repo commit 到兩台機器
- **舊位置處理**:`C:\Users\potato\.Codex\projects\E--memoir\memory\` 已清空,不再讀寫
...
| **LLM — Build-time 內容種子**(景點故事、脈絡、翻譯校對) | **Codex Pro / ChatGPT Plus / Gemini Advanced 訂閱版**(網頁版,人工操作) | — | 一次性產出,匯入 Content DB,App runtime 不需呼叫 |
...
> **訂閱 ≠ API**:ChatGPT Plus / Codex Pro / Gemini Advanced 是網頁版訂閱,不能直接被後端呼叫。

| 階段 | 用什麼 | 做什麼 |
|---|---|---|
| **Build-time(一次性,內容種子)** | Codex Pro / ChatGPT Plus / Gemini Advanced **訂閱版網頁** | 人工協作生成景點故事、跨景點脈絡、翻譯校對 → 匯入 Content DB |
...
## 9. 與 AI 協作的原則(給 Codex 的偏好)

> 這段直接寫給 Codex — 以下行為請主動遵守。
...
> **使用者請審閱本文件後,改名為 `AGENTS.md` 即生效。** 改名後 Codex 會在每次啟動時自動讀入。
